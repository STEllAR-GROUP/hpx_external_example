...
int hpx_main()
{
    ...
    std::cout << "Program will run on " << os_threads << " threads.\n"
              << "Increase or decrease the number of cores used by passing "
                 "the -tX argument to the program, where X is the number of "
                 "threads to run.\n";
    ...
    std::cout << "Transfer process completed in " << t.elapsed()
              << "s. Writing to hard disk...\n";
    t.restart();
    SetImage.WriteToFile("out.bmp");

    std::cout << "Fractal image written to file \"out.bmp\" from memory in "
